...
class SetClipModel:
    ...
    def image_label_detector(self,image_path,labels):
        ...
        image_emb=self.model.get_image_features(processed_image).detach().cpu().numpy() #embedding
        scores=np.dot(image_emb,SetClipModel().create_label_tokens(labels).T) #predicted lables scores (among the 39 original labels)
