...
class SetClipModel:
    ...
    def image_label_detector(self, image_path, labels):
        ...
        # embedding
        image_emb = self.model.get_image_features(processed_image).detach().cpu().numpy()

        # predicted lables scores (among the 39 original labels)
        scores = np.dot(image_emb, SetClipModel().create_label_tokens(labels).T)
...
if __name__ == "__main__":
    ...
    SetClipModel.image_label_detector(args.image_path, labels)
